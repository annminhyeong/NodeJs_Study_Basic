...
app.post('/chatroom', checkLogin , function(request, response){
    let chatSave = {
        title : 'testroom',
        member : [ObjectId(request.body.receiveId) , request.user._id],
        date : new Date()
    }
...
app.post('/message', checkLogin, function(request, response){
    ...
    db.collection('message').insertOne(saveDate).then((result)=>{
        response.send('DB 저장 성공');
    }).catch((error)=>{

    });
});

app.get('/message/:id',checkLogin , function(request, response){
